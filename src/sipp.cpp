...
static int bind_rtp_sockets(struct sockaddr_storage* media_sa, int try_port, int last_attempt)
{
    ...
    if ((media_socket_audio2 = socket(media_sa->ss_family, SOCK_DGRAM, IPPROTO_UDP)) == -1) {
    ...
    if (::bind(media_socket_audio2, (sockaddr*)media_sa, socklen_from_addr(media_sa)) != 0) {
        if (last_attempt) {
        ...
        ::close(media_socket_audio);
        media_socket_audio = media_socket_audio2 = -1;
        return -1;
    }
